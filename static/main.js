// GÃ©nÃ©rateur d'URL Grist - JavaScript principal
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Helper pour rÃ©cupÃ©rer l'API key
  const getApiKey = () => elements.apiKeyInput.value.trim();

  // Helper pour crÃ©er les headers avec l'API key
  const getHeaders = () => {
  ...
  elements.testApiBtn.addEventListener("click", async function () {
    ...
    console.log("Test de la clÃ© API lancÃ©");
    ...
    try {
      ...
      if (data.success) {
        showModal("Test rÃ©ussi", `âœ… ${data.message}`);
      } else {
        showModal(
          "Erreur de token API",
          `ProblÃ¨me avec le token API :\n\n${data.message}\n\nVÃ©rifiez votre token Grist`,
          true
        );
      }
    } catch (error) {
  ...
  // Fonction pour nettoyer les rÃ©sultats de test
  ...
  elements.loadTablesBtn.addEventListener("click", async function () {
    ...
    try {
      ...
      if (tables && tables.length > 0) {
        ...
        this.textContent = `âœ“ ${tables.length} tables chargÃ©es`;
        ...
        elements.tableSelect.innerHTML =
          '<option value="">-- Aucune table trouvÃ©e --</option>';
        showModal(
          "Aucune table trouvÃ©e",
          "Aucune table trouvÃ©e.\n\nVÃ©rifiez l'ID du document et votre token API",
          true
        );
      }
    } catch (error) {
  ...
  // Activation du bouton quand une colonne est sÃ©lectionnÃ©e
  ...
  // Fonction pour afficher les rÃ©sultats de succÃ¨s
  ...
  // Fonction pour afficher les rÃ©sultats d'erreur
  ...
  elements.form.addEventListener("submit", async function (e) {
    ...
    elements.result.style.display = "none";
    setButtonLoading(elements.generateBtn, true, "GÃ©nÃ©rer l'URL");
    ...
    try {
      ...
      console.error("ðŸ”¥ Erreur lors de la gÃ©nÃ©ration:", error);
  ...
  function copyToClipboard(text) {
    navigator.clipboard
      .writeText(text)
      .then(() => {
        showModal("CopiÃ© !", "URL copiÃ©e dans le presse-papiers !");
      })
      .catch((err) => {
  ...
  // Fonction pour tester l'URL gÃ©nÃ©rÃ©e
  async function testGeneratedUrl(url) {
    ...
    // Supprimer les rÃ©sultats de test prÃ©cÃ©dents
    const existingTestResults = document.querySelectorAll("[data-test-result]");
    existingTestResults.forEach((result) => result.remove());

    // Afficher le rÃ©sultat de test
  ...
  async function showTestResult(url, testId) {
    ...
    // Marquer comme rÃ©sultat de test pour pouvoir le supprimer
    const testContainer = document.createElement("div");
    testContainer.setAttribute("data-test-result", "true");
    testContainer.appendChild(clone);

    // Ajouter aprÃ¨s le rÃ©sultat principal
    ...
    accordionTitle.textContent = "Chargement des rÃ©sultats...";
    ...
    try {
      ...
      if (result.success) {
        ...
        // Mettre Ã  jour le titre de l'accordÃ©on
        accordionTitle.textContent = `RÃ©sultat JSON (${recordCount} enregistrement${
          recordCount > 1 ? "s" : ""
        } trouvÃ©${recordCount > 1 ? "s" : ""})`;
        ...
        // Ouvrir l'accordÃ©on pour montrer l'erreur
        accordionBtn.setAttribute("aria-expanded", "true");
        accordionCollapse.classList.add("fr-collapse--expanded");
      }
    } catch (error) {
    ...
    // RÃ©initialiser le JavaScript DSFR pour les nouveaux composants
    ...
    setTimeout(() => {
      if (typeof window.dsfr !== "undefined" && window.dsfr.accordions) {
        ...
        // Fallback : ajouter un gestionnaire d'Ã©vÃ©nements manuel
        initAccordionFallback(testContainer);
      }
    }, 100);
  }

  // Fonction fallback pour l'accordÃ©on si DSFR ne fonctionne pas
  function initAccordionFallback(container) {
    ...
    if (accordionBtn && accordionCollapse) {
      ...
      console.log("AccordÃ©on fallback initialisÃ©");
    }
  }
